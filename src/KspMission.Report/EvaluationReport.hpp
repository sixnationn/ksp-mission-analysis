...
nlohmann::json compose_shooting_report(const std::string& runtime_bytes,
    const std::string& expected_sha256,const nlohmann::json& request,
    const std::vector<nlohmann::json>& events);
void validate_shooting_report(const nlohmann::json& document);
void save_shooting_report(const nlohmann::json& document,const std::filesystem::path& path);
nlohmann::json load_shooting_report(const std::filesystem::path& path);
}
