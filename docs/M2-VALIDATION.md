...
- A persisted, versioned binary cache now round-trips complete synthetic ephemerides with source, frame, epoch, body descriptors, settings, measured fit errors, every sample and a SHA-256 integrity check. It rejects changed physics even when the caller keeps the same snapshot hash, and publishes without replacing an existing path. The 128 MiB cap is explicit. Local MSYS2 UCRT full CTest passed 14/14 on 23 September 2026; focused MSVC Debug cache tests passed 1/1. The `start_mission` worker can now create and reuse this cache through an explicit request path; see `M2-WORKER-CACHE-VALIDATION.md`. The single-leg `start` command and fixed-route evaluator still integrate in memory.
...
## Persisted cache validation, 23 September 2026

The failure cases and portable file format were specified in `M2-PERSISTED-CACHE-CONTRACT.md` before code. The owning tests compare every stored Cartesian sample and off-grid queries with the integrated synthetic source. They reject altered source confidence/hash, frame/epoch, body order, mu/radius/initial state, step/coverage/fit limit, an insufficient reload step cap, provisional and non-inertial callers, nonfinite inputs, an existing destination, bad magic/version, truncation/append, changed payload/checksum, a forged overlarge count with a valid checksum, and a nonfinite forged sample with a valid checksum. The test uses a unique disposable directory.

The loader verifies the same open file's length after reading, then validates the checksum and structure before returning any queryable object. Concurrent same-length in-place writes after reading are not locked against. SHA-256 detects damage; it does not authenticate the file or remeasure the stored fit errors. No runtime snapshot, JNSQ Reborn state, or Principia trajectory was produced by these tests. [CI run 35810236112](https://github.com/sixnationn/ksp-mission-analysis/actions/runs/35810236112) passed all six Ubuntu 24.04 and Windows 2022 jobs, including both C++ CTest matrices and desktop build checks, at source commit `05e1791`.
